...
// Shared API routes, mounted at /api by both entry points: app.js (local dev, via
// app.listen) and functions/api.js (Netlify Function, via serverless-http). Keeping the
// route logic here means the two environments can't drift out of sync with each other.
...
// than filtering in SQL. Registered before /location/:id so "nearby" isn't swallowed by
